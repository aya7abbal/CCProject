...
# Load datasets
transactions = pd.read_csv("/mnt/data/cleaned_transactions.csv")
households = pd.read_csv("/mnt/data/cleaned_households.csv")
products = pd.read_csv("/mnt/data/cleaned_products.csv")
...
def visualization():
    ...
    hshd_num = request.form.get("household_number", None)
    region = request.form.get("store_region", None)
    income_range = request.form.get("income_range", None)  # Added Income filter
    children = request.form.get("children", None)  # Presence of children filter
    loyalty = request.form.get("loyalty", None)  # Loyalty filter
    ...
    if region:
        filtered_data = filtered_data[filtered_data["STORE_R"] == region]
    if income_range:
        filtered_data = filtered_data[filtered_data["INCOME_DESC"] == income_range]
    if children:
        filtered_data = filtered_data[filtered_data["HAS_CHILDREN"] == children]
    if loyalty:
        filtered_data = filtered_data[filtered_data["LOYALTY"] == loyalty]
