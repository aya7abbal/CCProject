...
# Load the dataset
households_data = pd.read_csv('/mnt/data/cleaned_households.csv')

# Strip whitespace
households_data = households_data.applymap(lambda x: x.strip() if isinstance(x, str) else x)

# Replace 'null' strings with NaN
households_data = households_data.replace('null', np.nan)

# Optionally, save the cleaned data back to a CSV if needed
households_data.to_csv('./cleaned_households.csv', index=False)
