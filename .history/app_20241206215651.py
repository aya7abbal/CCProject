...
def visualization():
    ...
    income_group = request.form.get("income_group", None)
    loyalty_member = request.form.get("loyalty_member", None)

    filtered_data = merged_data

    # Apply filters to the data
    if hshd_num and hshd_num.isdigit():  # Check if hshd_num is not None and is a digit
        filtered_data = filtered_data[filtered_data["HSHD_NUM"] == int(hshd_num)]
    if region:
        filtered_data = filtered_data[filtered_data["STORE_R"] == region]
    if income_group:
        filtered_data = filtered_data[filtered_data["INCOME_DESC"] == income_group]
    if loyalty_member:
        filtered_data = filtered_data[filtered_data["LOYALTY"] == loyalty_member]
